 ...
 */
package com.jun.jokedaquan.widget.smarttablayout;
...
class SmartTabStrip extends LinearLayout {
    ...
    private static final boolean DEFAULT_INDICATOR_WIDTH_SAME_WITH_TEXT_WIDTH = false;
    ...
    private final boolean isIndicatorWidthSameWithTextWidth;
    ...
    SmartTabStrip(Context context, AttributeSet attrs) {
        ...
        boolean isIndicatorWidthSameWithTextWidth = DEFAULT_INDICATOR_WIDTH_SAME_WITH_TEXT_WIDTH;
        ...
        isIndicatorWidthSameWithTextWidth = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_indicatorWidthSameWithTextWidth, isIndicatorWidthSameWithTextWidth);
        a.recycle();
        ...
        this.isIndicatorWidthSameWithTextWidth = isIndicatorWidthSameWithTextWidth;
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            if (isIndicatorWidthSameWithTextWidth && !isLayoutRtl) {
                left = getSameWithLeft(selectedTab, left, right);
                right = getSameWithRight(selectedTab, left, right);
            }
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                if (isIndicatorWidthSameWithTextWidth && !isLayoutRtl) {
                    nextStart = getSameWithLeft(nextTab, nextStart, nextEnd);
                    nextEnd = getSameWithRight(nextTab, nextStart, nextEnd);
                }
    ...
    private int getSameWithLeft(View view, int left, int right) {
        if (view instanceof TextView) {
            int textWidth;
            TextView textView = (TextView) view;
            textWidth = (int) (textView.getPaint().measureText(textView.getText().toString()) + 0.5f);
            int sameWithLeft = left + (right - left - textWidth) / 2;
            return sameWithLeft;
        }
        return left;
    }

    private int getSameWithRight(View view, int left, int right) {
        if (view instanceof TextView) {
            int textWidth;
            TextView textView = (TextView) view;
            textWidth = (int) (textView.getPaint().measureText(textView.getText().toString()) + 0.5f);
            int sameWithRight = left + textWidth;
            return sameWithRight;
        }
        return right;
    }
